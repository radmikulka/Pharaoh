...
#include "Assets/_Shaders/Libraries/Common.hlsl"
...
half3 GetRimLight(half3 color, half amount, half contrast, half3 normalWs, half3 viewDirWSNormalized)
{
    float dotProductRim = 1 - saturate(dot(normalWs, viewDirWSNormalized));
    half rim = smoothstep(1 - amount, 1.0, dotProductRim) * contrast;
    //half rim = lerp(1 - amount, 1.0, dotProductRim) * contrast;
    return color * rim;
}
...
half3 CalculateBlinnPhong(Light light, InputData inputData, SurfaceData surfaceData, half specMod, half3 enviroColor)
{
    half3 attenuatedLightColor = light.color * (light.distanceAttenuation * light.shadowAttenuation);
    half3 lightDiffuseColor = LightingLambert(attenuatedLightColor, light.direction, inputData.normalWS);
    lightDiffuseColor += enviroColor;

    half3 lightSpecularColor = half3(0,0,0);
    #if defined(_SPECGLOSSMAP) || defined(_SPECULAR_COLOR)
    half smoothness = exp2(10 * surfaceData.smoothness + 1);

    lightSpecularColor += LightingSpecular(attenuatedLightColor, light.direction, inputData.normalWS, inputData.viewDirectionWS, half4(surfaceData.specular * specMod, 1), smoothness);
    #endif
...
/// Phong lighting...
////////////////////////////////////////////////////////////////////////////////
half4 UniversalFragmentBlinnPhong(InputData inputData, SurfaceData surfaceData, half specMod, half3 enviroColor)
{
    #if defined(DEBUG_DISPLAY)
    half4 debugColor;
...
#endif
    {
        lightingData.mainLightColor += CalculateBlinnPhong(mainLight, inputData, surfaceData, specMod, enviroColor);
    }
...
#endif
        {
            lightingData.additionalLightsColor += CalculateBlinnPhong(light, inputData, surfaceData, specMod, enviroColor);
        }
    }
    #endif
...
// Deprecated: Use the version which takes "SurfaceData" instead of passing all of these arguments...
half4 UniversalFragmentBlinnPhong(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness, half3 emission, half alpha, half3 normalTS, half specMod, half3 enviroColor)
{
    SurfaceData surfaceData;
    ...
    return UniversalFragmentBlinnPhong(inputData, surfaceData, specMod, enviroColor);
}
