...
namespace Pharaoh
{
	public class CFreezeRenderer : MonoBehaviour, IInitializable
	{
		...
		private CFreezeRendererFeature _rendererFeature;
		...
		{
			_eventBus.AddTaskHandler<CActivateFreezeRendererTask>(HandleActivateFreezeRenderer);
			_eventBus.AddTaskHandler<CDeactivateFreezeRendererTask>(HandleDeactivateFreezeRenderer);
			
			LoadRenderFeature();
			_rendererFeature.RefreshActivity(false);
		}

		private void HandleDeactivateFreezeRenderer(CDeactivateFreezeRendererTask task)
		{
			_lockObjects.Remove(task.LockObject);
			Refresh();
		}

		private void HandleActivateFreezeRenderer(CActivateFreezeRendererTask task)
		{
