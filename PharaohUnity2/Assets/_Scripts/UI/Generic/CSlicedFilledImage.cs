...
namespace TycoonBuilder.Ui
{
	...
	[CustomEditor(typeof(CSlicedFilledImage)), CanEditMultipleObjects]
	...
	public class CSlicedFilledImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement,
		ICanvasRaycastFilter
	{
		...
		private void GenerateSlicedFilledSprite(VertexHelper vh)
		{
			...
			for (int x = 0; x < 3; x++)
			{
				...
				for (int y = 0; y < 3; y++)
				{
					...
					float filledAmount = (m_FillAmount - sliceStart) / (sliceEnd - sliceStart);

					GenerateFilledSprite(vh, vertices, uvs, filledAmount);
				}
			}
		}
		...
		#if UNITY_2017_4 || UNITY_2018_2_OR_NEWER
		private static readonly List<CSlicedFilledImage> m_TrackedTexturelessImages = new List<CSlicedFilledImage>();
		...
		private static void RebuildImage(SpriteAtlas spriteAtlas)
		{
			...
			{
				CSlicedFilledImage image = m_TrackedTexturelessImages[i];
