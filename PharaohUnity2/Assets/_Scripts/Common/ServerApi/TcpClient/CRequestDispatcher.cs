...
namespace Pharaoh
{
	public class CRequestDispatcher : MonoBehaviour, IConstructable
	{
		...
		private CRequestQueue _hitsQueue;
		...
		private void LogOutputHits(CRequestRecord hitRecord)
		{
		...
		private void LogServerOutput(CRequestBatch recordsGroup)
		{
		...
		{
			CRequestRecord[] hits = _hitsQueue.GetNextRecordsGroupOrDefault();
			if(hits == null)
				return;
		...
		private async Task SendNext(CRequestRecord[] hits)
		{
			bool suppressErrorThrow = ContainsSuppressCommunicationErrorThrow(hits);
			CRequestBatch recordsGroup = new(_hitHeader, hits, suppressErrorThrow);
		...
		private void ProcessSuccessResponse(CRequestBatch recordsGroup, CResponsePacket response)
		{
			_mainThreadActions.Enqueue(() =>
			{
				for (int i = 0; i < response.Hits.Length; i++)
		...
		private bool ContainsSuppressCommunicationErrorThrow(CRequestRecord[] hits)
		{
		...
		private CRequestBatch TryInsertRefreshTokenHit(CRequestBatch recordsGroup)
		{
			bool insertRefreshTokenHit = ShouldInsertRefreshTokenHit(recordsGroup.Records);
			if(!insertRefreshTokenHit)
				return recordsGroup;

			CRequestRecord refreshTokenRecord = GetRefreshTokenRecord();

			List<CRequestRecord> newRecords = new List<CRequestRecord>
			{
				refreshTokenRecord
			};
			newRecords.AddRange(recordsGroup.Records);

			return new CRequestBatch(
				recordsGroup.Header, 
				newRecords.ToArray(),
		...
		private bool ShouldInsertRefreshTokenHit(CRequestRecord[] records)
		{
		...
		private CRequestRecord GetRefreshTokenRecord()
		{
			return new CRequestRecord(
				false, 
				null, 
				false, 
				false, 
