...
namespace Pharaoh
{
	public class CFrameCaptureRendererFeature : ScriptableRendererFeature
	{
		[Serializable]
		...
		private class CRenderPass : ScriptableRenderPass
		{
			...
			public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
			{
				...
				BlitMaterialParameters blitParams = new(frame, destination, CBlitMaterial.Material, 0);
		...
		public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
		{
			...
			bool isFreezeCamera = renderingData.cameraData.camera.GetComponent<CFrameCaptureCamera>() != null;
			if(!isFreezeCamera)
				return;
			
			if (!_isEnabled)
			{
				CaptureFrame(renderingData.cameraData.camera, false);
				if (!_renderLastFrame)
				{
					return;
				}
			...
			if (_captureNextFrame)
			{
				...
				CaptureFrame(renderingData.cameraData.camera, true);
			}
		...
		private void CaptureFrame(Camera camera, bool state)
		{
			IMainCamera mainCamera = camera.GetComponent<IMainCamera>();
			mainCamera?.CaptureFrame(state);
		}
