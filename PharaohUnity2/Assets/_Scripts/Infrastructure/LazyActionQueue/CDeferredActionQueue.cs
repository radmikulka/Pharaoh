...
namespace Pharaoh
{
	public class CDeferredActionQueue : MonoBehaviour, IAldaFrameworkComponent
	{
		private readonly List<IDeferredAction> _actionsToExecute = new();
		...
		public void AddAction(IDeferredAction action)
		{
		...
		{
			CMenuStateResponse response = _eventBus.ProcessTask<CMenuStateRequest, CMenuStateResponse>();
			if (response == null)
				return false;
